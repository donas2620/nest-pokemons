...
export class PokemonService {
  ...
  async update(term: string, updatePokemonDto: UpdatePokemonDto) {
    ...
    try{
      ...
      return {...pokemon.toJSON(), ...updatePokemonDto}; //Mandamos toda la info que se obtiene del JSON que aun no esta actualizada, y sobreescribirle la info actualizada
    } catch (error){
      this.handleExceptions(error);
    }
  }
  ...
  private handleExceptions(error: any){
    if(error.code === 11000){
      throw new BadRequestException(`Pokemon exists in db ${JSON.stringify(error.keyValue)}`); /*Validar mismos datos en base de datos */
    }
    console.log(error);
